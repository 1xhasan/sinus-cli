...
async function main() {
    ...
    const { keyword } = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().toLowerCase() === "solana" ||
    ...
    switch (action) {
        case "transfer":
            await handleTransfer(cluster);
            break;
        case "airdrop":
            await handleAirdrop(cluster);
            break;
        default:
...
// ========== ACTION HANDLERS ==========
async function handleTransfer(cluster) {
    ...
    const { recipient, amount, secretKeyPath } = await inquirer.prompt([
        {
            ...
            validate: (input) => input && input.length > 30 ? true : "Invalid address",
        },
        {
            type: "number",
            name: "amount",
...
async function handleAirdrop(cluster) {
    const connection = new Connection(clusterApiUrl(cluster), "confirmed");
    ...
    try {
        const sig = await connection.requestAirdrop(new PublicKey(publicKey), amount * LAMPORTS_PER_SOL);
